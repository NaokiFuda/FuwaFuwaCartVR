...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        protected virtual IEnumerator Start()
        {
            ...
            HandColliders = GetComponent<HandPhysics>().handCollider.GetComponentsInChildren<Collider>();
        }

        [SerializeField] bool useFingerCollider;
        ...
        protected virtual void UpdateHovering()
        {
            ...
            if (useFingerCollider && mainRenderModel != null && mainRenderModel.IsHandVisibile())
            {
                foreach(SphereCollider handCollider in HandColliders)
                {
                    float scaledHoverRadius = handCollider.radius * Mathf.Abs(SteamVR_Utils.GetLossyScale(handCollider.transform));
                    CheckHoveringForTransform(handCollider.transform.position, scaledHoverRadius * 1.2f, ref closestDistance, ref closestInteractable, Color.yellow);
                }
